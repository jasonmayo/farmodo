...
pathToDB = "/Volumes/blotto_studio/blottoweb/database/blottoTestFarmDB.db"
...
def renderJob(job):    #render.animation this job
    ...
    try:
        ...
        for eachInitialPass in job[INITIAL_PASS].split(','):
            rpg = rpgExists(eachInitialPass.split(':')[0])
            if rpg:
                print eachInitialPass
                rpg.active = True
                initialPassesPattern += eachInitialPass + "_"
        ...
        print "replacing output pattern"
        outPat = outPat.replace('<camera>',renderCam)
        outPat = outPat.replace('<initialPass>',initialPassesPattern)
        outPat = outPat.replace('__','_')
        outPat = outPat.replace('___','_')
        outPat = outPat.replace('[','')
        outPat = outPat.replace(']','')
        scene.items('polyRender')[0].channel('outPat').set(outPat)
        lx.out("Output pattern changed to :%s " % (scene.items('polyRender')[0].channel('outPat').get()))




        lx.out("pass list specified %s" % job[PASSGRP])
        if ("None" in job[PASSGRP]) or (job[PASSGRP]==''):
            print "No Passes specified, passgrp ignored, rendering default scene state with no passes"
        else:
...
def jobAssignedToThisNode():
    ...
    print "finding job assigned to this node"
    ...
    pragma =cur.fetchall()
    cur.execute('SELECT * FROM renderJobs WHERE username = "%s" AND status = "%s" '  % (username,"assigned"))
    job=cur.fetchone()
    conn.close()
    if job:
        for i in range(len(job)):
            jobAsDict[pragma[i][1]]=job[i]
    return jobAsDict
